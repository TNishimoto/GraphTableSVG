window.onload = () => {
    ...
    for(let i=0;i<dashStyles.length;i++){
        ...
        const edgeL = GraphTableSVG.Edge.create(graph, {beginVertex : nodeLL, endVertex : nodeL, text : dashStyles[i], pathTextAlignment : "regularInterval" });
        const edgeC = GraphTableSVG.Edge.create(graph, {beginVertex : nodeL, endVertex : nodeC});
        const edgeR = GraphTableSVG.Edge.create(graph, {beginVertex : nodeC, endVertex : nodeR});

        edgeL.svgPath.style.strokeWidth="1"; 
        edgeC.svgPath.style.strokeWidth="3"; 
        edgeR.svgPath.style.strokeWidth="6";
